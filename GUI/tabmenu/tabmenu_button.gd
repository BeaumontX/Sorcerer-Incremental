...
func SetBorderSize() -> void:
	...
	if top:
		...
		newstyle.border_width_top = border_size/2
	
	if bottom:
		newstyle.border_width_bottom = border_size
	else:
		newstyle.border_width_bottom = border_size/2
	
	newstyle.border_width_left = border_size
	
	if selected:
		newstyle.border_width_right = 0
	else:
...
func Modulate() -> void:
	if selected:
		...
		texture.modulate = Color(modulate_amount, modulate_amount, modulate_amount)
